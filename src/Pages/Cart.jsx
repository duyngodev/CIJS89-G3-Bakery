...
import { removeFromCart,increaseQuantity,decreaseQuantity  } from "../Components/nhandev/actions.jsx";
...
const Cart = (props) => {
    ...
    const increaseBtClickHandler = (cartItem, cart) => {
    ...
    const cartsCheckAmountNoEmpty = data?.cart.filter((cartItem) => cartItem.quantityInCart > 0);
    
    if (!data || !data.cart || data.cart.length === 0 || !cartsCheckAmountNoEmpty || cartsCheckAmountNoEmpty.length === 0) {
        return (
            <Grid container spacing={2} columns={16} sx={{
                ...
                <Grid item lg={7} sm={7} xs={16}>
                    <h2 style={{ textAlign: 'center', fontSize: '18px' }}>Giỏ hàng của tôi ({data?.cart?.length || 0})</h2>
                ...
                <Grid item lg={6} sm={5} xs={16} sx={{
                    ...
                    <Grid container spacing={3}>
                        ...
                        <Grid item lg={4} md={8} sm={7.6} xs={9}>
                            <p style={{ textAlign: 'right' }}>{currencyFormatter.format(totalPrice, { code: 'VND' })}</p>
                        </Grid>
                    </Grid>
                    <Grid container spacing={3}>
                        <Grid item lg>
                        ...
                        <Grid item lg={4} md={6.8} sm={6} xs={8}>
                            <p style={{ textAlign: 'right' }}>{currencyFormatter.format(voucher, { code: 'VND' })}</p>
                        </Grid>
                    </Grid>
                    <Grid container spacing={3}>
                        <Grid item lg sm={5.5}>
                        ...
                        <Grid item lg={4} md={4.5} sm xs={6}>
                            <p style={{ textAlign: 'right' }}>{currencyFormatter.format(totalPrice-voucher, { code: 'VND' })}</p>
                        </Grid>
                    </Grid>
                    <hr style={{ backgroundColor: 'black', borderColor: 'transparent', borderWidth: '0.5px' }} />
                    <Stack spacing={2} sx={{ marginTop: '20px' }}>
                        <ThemeProvider theme={theme}>
                            <Button variant="contained" sx={{
                                ...
                                <Link to="/products/20-11" style={{ color: 'white' }}>Tiếp tục mua hàng</Link>
                            </Button>
                        </ThemeProvider>
                    </Stack>
                </Grid>
                                        ...
                                        <Box key={cartItem.id} sx={{ flexGrow: 1 }}>
                                            <Grid container spacing={2}>
                                                ...
                                                <Grid item lg={2.3} md={2.3} sm={2.3} xs={2.2}>
                                                    <p>{currencyFormatter.format(cartItem.price, { code: 'VND' })}</p>
                                            </Grid>
                                            <Grid item lg={2.2} md={2.2} sm={2.3} xs={2}>
                                                <Stack direction="row" spacing={1} >
                                                    ...
                                                    <AddCircle fontSize='small' sx={{
                                                        ...
                                                        width: 'auto',
                                                        transition: 'all 300ms ease'
                                                    }} onClick={() => increaseBtClickHandler(cartItem, data.cart)} />
                                                </Stack>
                                            </Grid>
                                            <Grid item lg={2} md={2} sm={2.3} xs={2.5}>
                                                <p style={{ textAlign: 'end' }}>{currencyFormatter.format(cartItem.quantityInCart * cartItem.price, { code: 'VND' })}</p>
                                            </Grid>
                                            <Grid item lg={0.6}>
